...
import { FailedToCreateProjectEvent } from '@projects/events/impl/failed-to-create-project.event';
...
export class CreateProjectHandler extends BaseSyncCommandHandler<CreateProjectCommand, Project> {
    ...
    private async getClientById(id: string): Promise<Result<Client>> {
        ...
        const client = await this.clientRepository.findById( beneficiaryId.value! );
