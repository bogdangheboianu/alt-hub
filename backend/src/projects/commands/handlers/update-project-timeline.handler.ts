...
import { FailedToUpdateProjectTimelineEvent } from '@projects/events/impl/failed-to-update-project-timeline.event';
import { ProjectTimelineUpdatedEvent } from '@projects/events/impl/project-timeline-updated.event';
...
@CommandHandler( UpdateProjectTimelineCommand )
export class UpdateProjectTimelineHandler extends BaseSyncCommandHandler<UpdateProjectTimelineCommand, Project> {
    constructor(
    ...
    async execute(command: UpdateProjectTimelineCommand): Promise<Result<Project>> {
        ...
        const updatedProject: Result<Project> = projectToUpdate.value!.updateTimeline( command );
    ...
    protected failed(command: UpdateProjectTimelineCommand, ...errors: IException[]): Result<Project> {
        const { context } = command.data;
        const event = new FailedToUpdateProjectTimelineEvent( { context, errors } );
    ...
    protected successful(command: UpdateProjectTimelineCommand, project: Project): Result<any> {
        const { context } = command.data;
        const event = new ProjectTimelineUpdatedEvent( { context, payload: project } );
    ...
    private async getProjectById(command: UpdateProjectTimelineCommand): Promise<Result<Project>> {
