...
export class CompanyPosition implements IDomainModel<CompanyPosition, CompanyPositionEntity> {
    ...
    static create(command: CreateCompanyPositionCommand): Result<CompanyPosition> {
    ...
    static fromName(command: IDomainCommand<any>, name: string): Result<CompanyPosition> {
        const buildData = Result.aggregateObjects<Pick<ICompanyPosition, 'name' | 'audit'>>(
            { name: CompanyPositionName.create( name, 'name' ) },
